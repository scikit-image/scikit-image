...
def check_radon_iradon(interpolation_type, filter_type, image):
    debug = False
...
def test_radon_iradon(interpolation_type, filter_type, _phantom):
    check_radon_iradon(interpolation_type, filter_type, _phantom)
...
@run_in_parallel()
def test_iradon_sart(_phantom):
    debug = False

    image = rescale(
        _phantom, 0.8, mode='reflect', channel_axis=None, anti_aliasing=False
    )
...
def test_radon_dtype(_phantom):
    img = convert_to_float(_phantom, False)
