...
class Test_binary_blobs:
    def test_volume_fraction(self):
        blobs = binary_blobs(shape=(128, 128), blob_size=13)
        assert_almost_equal(blobs.mean(), 0.5, decimal=1)

        blobs = binary_blobs(shape=(128, 128), blob_size=13, volume_fraction=0.25)
        assert_almost_equal(blobs.mean(), 0.25, decimal=1)

        blobs = binary_blobs(shape=(32, 32, 32), blob_size=3, volume_fraction=0.25)
        assert_almost_equal(blobs.mean(), 0.25, decimal=1)

        other_realization = binary_blobs(
            shape=(32, 32, 32), blob_size=3, volume_fraction=0.25
        )
    ...
    def test_boundary(self):
        ...
        blobs_near = binary_blobs(
            shape=(300, 300), blob_size=30, boundary_mode="nearest", rng=3
        )
        blobs_wrap = binary_blobs(
            shape=(300, 300), blob_size=30, boundary_mode="wrap", rng=3
        )
