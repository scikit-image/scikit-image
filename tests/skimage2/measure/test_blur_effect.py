...
def test_blur_effect():
    ...
    B2 = ski2.measure.blur_effect(
        ski2.filters.gaussian(image, sigma=4, channel_axis=-1), channel_axis=-1
    )
    assert 0 <= B0 < 1
    assert B0 < B1 < B2
...
def test_blur_effect_uniform_input(factor):
    ...
    B = ski2.measure.blur_effect(image, channel_axis=-1)
    assert B == 1
...
def test_blur_single_axis_constant_image(shape):
    ...
    B = ski2.measure.blur_effect(image)
    assert B == 1
