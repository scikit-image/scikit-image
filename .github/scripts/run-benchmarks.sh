...
# resolved baseline, and fail the step if a regression or error is detected.
#
# Expects prepare-benchmarks.sh to have run first; it puts these into the
# job environment from benchmark-params.json:
...
#
# ASV_SKIP_SLOW and ASV_FULL_PARAMS come from the same file but are read by
# benchmarks/__init__.py in the benchmark processes, not here.
#
