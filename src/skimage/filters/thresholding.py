...
def threshold_circular_otsu(image=None, nbins=256, *, val_range, hist=None):
    ...
    References
    ----------
    .. [2] Preprint PDF, https://users.cs.cf.ac.uk/Yukun.Lai/papers/thresholdingTIP.pdf
    ...
    if hist is None:
        count, bin_edges = np.histogram(image, bins=nbins, range=val_range)
    ...
    x_min = x[i_min]
    return x_min, x_min + 0.5 * (val_range[1] - val_range[0])
