...
from _skimage2._shared.interpolation cimport get_pixel3d, get_pixel4d
...
def _denoise_bilateral3d(np_floats[:, :, :, ::1] image, cnp.float64_t max_value,
                       Py_ssize_t win_size, cnp.float64_t sigma_color,
                       cnp.float64_t sigma_spatial, Py_ssize_t bins, mode,
                       cnp.float64_t cval, np_floats[::1] color_lut,
                       np_floats[::1] range_lut, np_floats[::1] empty_dims,
    ...
    cdef:
        Py_ssize_t rows = image.shape[0]
        Py_ssize_t cols = image.shape[1]
        Py_ssize_t depth = image.shape[2]
        Py_ssize_t dims = image.shape[3]
        Py_ssize_t window_ext = (win_size - 1) / 2
        Py_ssize_t max_color_lut_bin = bins - 1

        Py_ssize_t r, c, z, d, wr, wc, wz, kr, kc, kz, rr, cc, zz, color_lut_bin
        np_floats value, weight, dist, total_weight, csigma_color, color_weight, \
               range_weight, t
        np_floats dist_scale
        np_floats[:] values
        ...
        for r in range(rows):
            for c in range(cols):
                for z in range(depth):
                    ...
                    for d in range(dims):
                        total_values[d] = 0
                        centres[d] = image[r, c, z, d]

                    for wz in range(-window_ext, window_ext + 1):
                        zz = wz + z
                        kz = wz + window_ext
                        for wr in range(-window_ext, window_ext + 1):
                            rr = wr + r
                            ...
                            for wc in range(-window_ext, window_ext + 1):
                                ...
                                dist = 0
                                for d in range(dims):
                                    value = get_pixel4d(&image[0, 0, 0, 0], rows, cols, depth,
                                                        dims, rr, cc, zz, d, cmode, cval)
                                    values[d] = value
                                    t = centres[d] - value
                                    dist += t * t
                                dist = sqrt(dist)

                                range_weight = range_lut[kz * win_size * win_size + kr * win_size + kc]
