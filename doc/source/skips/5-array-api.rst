...
using low-level implementations of hardware accelerated algorithms [4].
...
SciPy [6] and scikit-learn [7], are implementing a similar adoption process, and this
...
See [8, 9] for a discussion and worked examples, and/or [14] for a SciPy-specific 
discussion.
...
Note that this pattern is zero-copy for arrays on CPU [11], and raises a clear error
if converting to NumPy requires a device transfer. See [14] for a detailed discussion.
...
- maintain parallel "backends": typically this means writing a "generic", pure python
  ...
  the performance benefits (if any). See [4] and [12] for demos of the latter.
- For functions implemented in ``pythran``, use the pythran's "dual mode" [13], where
