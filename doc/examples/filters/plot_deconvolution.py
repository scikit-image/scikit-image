...
In this example, we discuss three deconvulation algorithms and implement two of them.
...
3. Richardson-Lucy deconvolution algorithm
   ---------------------------------------
...
NB: The above algorithms are based on linear models that can't restore sharp edge as much as non-linear
methods (like TV restoration) but are much faster.

We now deconvole a noisy version of an image using
a) Unsupervised Wiener algorithm 
...
#Restore image using Unsupervised Wiener
psf = np.ones((5, 5)) / 25
...
b) Richardson-Lucy deconvolution algorithm
