...
ax[0, 3].axis('off')
ax[0, 3].set_title('Wavelet in RGB')
...
denoised_y_channel = denoise_wavelet((noisy_ycbcr[..., 0] - 16) / (235 - 16))
...
denoised_in_ycbcr = np.clip(denoised_in_ycbcr, 0, 1)

ax[1, 3].imshow(denoised_in_ycbcr)
ax[1, 3].axis('off')
ax[1, 3].set_title('Wavelet denoising in YCbCr')
ax[1, 0].imshow(astro)
ax[1, 0].axis('off')
