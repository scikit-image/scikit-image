...
import skimage.segmentation._graph_cut as gc
reload(gc)
img = img_as_float(lena()[::2, ::2])
segments_fz = felzenszwalb(img, scale=100, sigma=0.5, min_size=50)
segments_slic = slic(img, ratio=10, n_segments=250, sigma=1)
segments_quick = quickshift(img, kernel_size=3, max_dist=6, ratio=0.5)
segments_graph_cut = gc.normalized_cut(img, mask="slic", n_cluster = 10, n_init_cluster=100)
...
ax[3].imshow(mark_boundaries(img, segments_graph_cut))
ax[3].set_title("Graph Cut")
for a in ax:
    a.set_xticks(())
    a.set_yticks(())
plt.show()
