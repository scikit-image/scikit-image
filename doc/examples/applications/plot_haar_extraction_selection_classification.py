...
if __name__ == '__main__':
    ...
    ###########################################################################
    # The procedure to extract the Haar-like features for an image is quite
    # easy. Firstly, a region of interest (ROI) is defined for which all
    # possible features will be extracted. Secondly, the integral image of
    # this ROI will be computed and used to compute the features.
    ...
    ###########################################################################
    # We will use a subset of the CBCL which is composed of 100 face images and
    # 100 non-face images. Each image has been resized to a ROI of 19 by 19
    # pixels. We will keep 75 images from each group to train a classifier and
    # check which extracted features are the most salient, and use the
    # remaining 25 from each class to check the performance of the classifier.
    ...
    # check which features are most oftenly used by the ensemble of trees.
    # By using only the most salient features in subsequent steps, we can
    # dramatically speed up computation, while retaining accuracy.
    ...
    # of the feature importance index. Below, we keep features representing 70%
    # of the cumulative value which represent only 3% of the total number of
    # features.
    ...
    # Note: we could select those features from the original matrix X but we
    # would like to emphasize the usage of `feature_coord` and `feature_type`
    ...
    # When computing all features, the Python GIL is acquired to process each
    # ROI, and this is where most of the time is spent, so multiprocessing is
    # faster. For this small subset, most of the time is spent on the feature
    # computation rather than the ROI scanning, and using threaded is *much*
    # faster, because we avoid the overhead of launching a new process.
    t_start = time()
