...
segmenter = future.TrainableSegmenter(clf=clf, features_func=features_func)
...
l = len(_clf.feature_importances_)
feature_importance = (
        _clf.feature_importances_[: l//3],
        _clf.feature_importances_[l//3: 2*l//3],
        _clf.feature_importances_[2*l//3:])
sigmas = np.logspace(
...
result_new = segmenter.predict(img_new)
