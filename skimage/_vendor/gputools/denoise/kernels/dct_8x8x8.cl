...
dct_denoise(read_only image3d_t src, global float *acc, float thr, int dx, int dy, int dz, int astride, int bstride,int set) {
	const sampler_t smp = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
	local float ldata[8*DFSTRIDE*DFSTRIDE];
	...
	ldata[lx + ly*DFSTRIDE+lz*DFSTRIDE*DFSTRIDE] = v;
	
	barrier(CLK_LOCAL_MEM_FENCE);

	// dct_forward_threshold(ldata, ly*DFSTRIDE, lz, ly, lx, DFSTRIDE, thr);
