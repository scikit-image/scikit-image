...
class TestColorconv:
    ...
    def test_xyz2lab(self):
        ...
        for I in ["A", "B", "C", "d50", "d55", "d65"]:
            ...
            for obs in ["2", "10", "R"]:
                obs = obs.lower()
                fname = f'color/tests/data/lab_array_{I}_{obs}.npz'
                lab_array_I_obs = np.load(fetch(fname))['data']
                assert_array_almost_equal(
                    lab_array_I_obs, xyz2lab(self.xyz_array, I, obs), decimal=2
                )
        for I in ["d75", "e"]:
            fname = f'color/tests/data/lab_array_{I}_2.npz'
            lab_array_I_obs = np.load(fetch(fname))['data']
            assert_array_almost_equal(
    ...
    def test_xyz2luv(self):
        ...
        for I in ["A", "B", "C", "d50", "d55", "d65"]:
            ...
            for obs in ["2", "10", "R"]:
                obs = obs.lower()
                fname = f'color/tests/data/luv_array_{I}_{obs}.npz'
                luv_array_I_obs = np.load(fetch(fname))['data']
                assert_array_almost_equal(
                    luv_array_I_obs, xyz2luv(self.xyz_array, I, obs), decimal=2
                )
        for I in ["d75", "e"]:
            fname = f'color/tests/data/luv_array_{I}_2.npz'
            luv_array_I_obs = np.load(fetch(fname))['data']
            assert_array_almost_equal(
