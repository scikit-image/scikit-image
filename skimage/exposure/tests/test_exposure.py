...
@pytest.mark.parametrize("input_image, source_range", [
    (data.camera(), 'image'),
    (data.camera(), 'dtype'),
])
def test_dask_histogram():
    ...
    dask_array = da.from_array(input_image, chunks=(256, 256))
    output_hist, output_bins = exposure.histogram(dask_array,
                                                  source_range=source_range)
    expected_bins = np.array(list(range(256)))
    expected_hist = np.array(
        [ 102,   76,   89,  114,  159,  209,  335, 1173, 3523, 5129, 4490,
