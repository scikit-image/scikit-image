...
def rescale_intensity(image, in_range='image', out_range='dtype'):
    """
    Rescale the intensity values of an image.

    This function adjusts the intensity values in `image` from the specified
    input range (`in_range`) to the desired output range (`out_range`).
    It performs linear scaling of the values and clips those outside the
    input range.
    ...
    image : array
        Input image array.
    in_range : {'image', 'dtype', dtype-name} or 2-tuple of scalars, optional
        Intensity range of the input image:
        - 'image': Use min and max of the input image.
        - 'dtype': Use full intensity range of the image's dtype.
        - dtype-name: Use intensity limits of the specified dtype (e.g., 'uint8').
        - tuple (min, max): Specify explicit min and max values.

    out_range : {'dtype'} or 2-tuple of scalars, optional
        Desired intensity range of the output image:
        - 'dtype': Use full range of the image's dtype.
        - tuple (min, max): Specify explicit output intensity range.

    Returns
    -------
    out : array
        Image array with rescaled intensity values. Output dtype matches the input
        unless a custom range requires float.

    Notes
    -----
    .. versionchanged:: 0.17
        The output array's dtype now matches the input dtype unless a float range is used.
