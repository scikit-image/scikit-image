...
class TestGuidedFilter(object):
    ...
    def test_self_guided(self):
        ...
        assert_allclose(guided, guided_self, rtol=0, atol=1e-6)
    
    def test_smoothing(self):
        """Test that increasing eta reduces variance in image."""
        etas = [0.01, 0.1, 1]
        var_baseline = self.noise.var()
        variances = np.array([_guided_filter(self.noise, eta, 5) for eta in etas])
        assert (variances < var_baseline).all()
    
    def test_large_eta_limit(self):
        """Test that smoothing with large eta approaches an average filter."""
        guided = _guided_filter(self.image, 10, 10)
        uniform = uniform_filter(self.image, 10)
        assert_allclose(guided, uniform, rtol=0, atol=1e-6)
    ...
    def test_too_many_dimensions(self):
        ...
        guide_dict = dict(guide = four_d)
        assert_raises(ValueError, guided_filter, one_d, 0.1, 5, guide_dict)
        assert_raises(ValueError, guided_filter, four_d, 0.1, 5, guide_dict)
    
    def test_incompatible_guide(self):
        """Test error is raised for incompatible channels in image and guide."""
        guide_dict = dict(guide = self.multichannel[..., :2])
