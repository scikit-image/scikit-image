...
def guided_filter(image, eta, radius, guide=None):
    ...
    if guide is not None:
        if image.shape[:2] != guide.shape[:2]:
            raise ValueError("Image and guide must have the same first two dimensions.")
        if image.ndim == guide.ndim == 2:
            ...
            for channel in range(image.shape[2]):
                output[..., channel] = _guided_filter(image[..., channel].astype('float'),
                                                      guide.astype('float'),
                                                      eta, radius)
            return output
            ...
            if image.shape[2] != guide.shape[2]:
                raise ValueError("Image and guide must have the same number of channels.")
            output = np.empty(image.shape)
            for channel in range(image.shape[2]):
                output[..., channel] = _guided_filter(image[..., channel].astype('float'),
        ...
        if image.ndim == 2:
            ...
            output = np.empty(image.shape)
            for channel in range(image.shape[2]):
                output[..., channel] = _guided_filter_same(image[..., channel].astype('float'),
                                                           eta, radius)
            return output
        else:
