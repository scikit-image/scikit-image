...
cdef inpaint_point(Py_ssize_t i, Py_ssize_t j, cnp.uint8_t[:, ::1] image_view,
            cnp.uint8_t[:, ::1] flag_view, cnp.float_t[:, ::1] u_view, Py_ssize_t epsilon):
...
cdef cnp.float_t eikonal(Py_ssize_t i1, Py_ssize_t j1, Py_ssize_t i2, Py_ssize_t j2, cnp.uint8_t[:, ::1] flag_view, cnp.float_t[:, ::1] u_view):
...
cpdef object fast_marching_method(cnp.uint8_t[:, ::1] image_view, cnp.uint8_t[:, ::1] flag_view,
                cnp.float_t[:, ::1] u_view, heap, _run_inpaint=True, Py_ssize_t epsilon=5):
    ...
    while len(heap):
        ...
        for (i_nb, j_nb) in (i - 1, j), (i, j - 1), (i + 1, j), (i, j + 1):
            ...
            if not flag_view[i_nb, j_nb] == KNOWN:
                u_view[i_nb, j_nb] = min(eikonal(i_nb - 1, j_nb, i_nb, j_nb - 1, flag_view, u_view),
                                                    eikonal(i_nb + 1, j_nb, i_nb, j_nb - 1, flag_view, u_view),
                                                    eikonal(i_nb - 1, j_nb,  i_nb, j_nb + 1, flag_view, u_view),
                                                    eikonal(i_nb + 1, j_nb, i_nb, j_nb + 1, flag_view, u_view))
                ...
                if flag_view[i_nb, j_nb] == INSIDE:
                    ...
                    if _run_inpaint:
                        inpaint_point(i_nb, j_nb, image_view, flag_view, u_view, epsilon)
