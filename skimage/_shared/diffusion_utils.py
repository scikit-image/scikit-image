...
def nonlinear_iso_step(image, out_image, tau, gradX, gradY, alpha, type):
    ...
    for i in range(1, image.shape[0] - 1):
        for j in range(1, image.shape[1] - 1):
            ...
            out_image[i, j] = (tau * (diff_ij + get_diffusivity(gradX[i + 1, j], gradY[i + 1, j], alpha, type)) / 2 * image[i + 1, j] / np.power(h1, 2)) + (tau * (diff_ij + get_diffusivity(
                gradX[i - 1, j],
                gradY[i - 1, j],
                alpha, type)) / 2 * image[i - 1, j] / np.power(h1, 2)) + (tau * (diff_ij + get_diffusivity(
                    gradX[
                        i, j + 1],
                    gradY[
                        i, j + 1],
                    alpha, type)) / 2 * image[i, j + 1] / np.power(h2, 2)) + (tau * (diff_ij + get_diffusivity(
                        gradX[
                            i, j - 1],
                        gradY[
                            i, j - 1],
                        alpha, type)) / 2 * image[i, j - 1] / np.power(h2, 2)) + (1 - (tau * (diff_ij + get_diffusivity(
                            gradX[
                                i + 1, j],
                            gradY[
                                i + 1, j],
                            alpha, type)) / 2 / np.power(h1, 2)) - (tau * (diff_ij + get_diffusivity(
                                gradX[
                                    i - 1, j],
                                gradY[
                                    i - 1, j],
                                alpha, type)) / 2 / np.power(h1, 2)) - (tau * (diff_ij + get_diffusivity(
                                    ...
                                    gradY[
                                        i, j + 1],
                                    alpha, type)) / 2 / np.power(h2, 2)) - (tau * (diff_ij + get_diffusivity(
                                        gradX[
                                            i, j - 1],
                                        gradY[
                                            i, j - 1],
                                        alpha, type)) / 2 / np.power(h2, 2))) * image[i, j]
...
def linear_step(image, out_image, tau):
    ...
    for i in range(1, image.shape[0]-1):
        for j in range(1, image.shape[1] - 1):
            out_image[i, j] = image[i, j] + tau * (((image[i+1, j] - 2*image[i, j] + image[i-1, j]) / np.power(
                h1, 2)) + ((image[i, j+1] - 2*image[i, j] + image[i, j-1]) / np.power(h2, 2)))
...
def prepare_diagonals(Diag, UDiag, LDiag, data, data_i, n, tau, shift, sx):
    ...
    for i in range(1, n - 1):
        UDiag[i] = -tau * (data[get_coord(data_i, sx)] +
                           data[get_coord(data_i + shift, sx)])
        LDiag[i] = UDiag[i]
        Diag[i] = 1 - UDiag[i] - UDiag[i - 1]
        data_i += shift
    Diag[n - 1] = 1 - UDiag[n - 2]
...
def add_and_avg(src, dst, dst_i, n, step, coef, sx):
    ...
    """
    for i in range(n):
        dst[get_coord(dst_i, sx)] += src[i]
        dst[get_coord(dst_i, sx)] *= coef
        dst_i += step


@jit(nopython=True)
def aniso_diff_step_AOS(img, Da, Db, Dc, out, tau):
    sx = img.shape[1]  # number of cols
    sy = img.shape[0]  # number of rows
    n = max(sx, sy)
    r = np.zeros(n)
    Diag = np.zeros(n)
    UDiag = np.zeros(n - 1)
    ...
    for y in range(1, sy - 1):
        ...
        for x in range(1, sx - 1):
            tmp_img[get_coord(tmp_i, sx)] = img[get_coord(u, sx)] + 0.25 * tau * (
    ...
    for y in range(sy):
        r = tmp_img[tmp_i//sx, :].copy()  # copy ith row
        prepare_diagonals(Diag, UDiag, LDiag, Da, a, sx, tau, 1, sx)
    ...
    for x in range(sx):
        ...
        r = tmp_img[:, tmp_i].copy()  # copy ith column
        prepare_diagonals(Diag, UDiag, LDiag, Dc, c, sy, tau, sx, sx)
        tridiagonal_matrix_solver(Diag, UDiag, LDiag, r,  solution, sy)
        add_and_avg(solution, out, out_i, sy, sx, 0.5, sx)
...
def tridiagonal_matrix_solver(diag_a, diag_b, diag_c, d, x, n):
    ...
    for i in range(n - 1):
        diag_c[i] /= diag_a[i]
        diag_a[i + 1] -= diag_c[i] * diag_b[i]

    for i in range(1, n):
        d[i] -= diag_c[i - 1] * d[i - 1]

    x[n - 1] = d[n - 1] / diag_a[n - 1]

    for i in range(n-2, -1, -1):
        x[i] = (d[i] - diag_b[i] * x[i + 1]) / diag_a[i]
