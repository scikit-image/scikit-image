...
def harmonc(image, sigma=1, output=None, mode='nearest', cval=0,
             preserve_range=False, truncate=4.0, *,
             channel_axis=None):
    """Multi-dimensional Harmonic filter.
    ...
    array([[0., 0., 0.],
           [0., 1., 0.],
           [0., 0., 0.]])
    >>> harmonc(a, sigma=0.4)  # mild smoothing
    array([[0.00163116, 0.03712502, 0.00163116],
           [0.03712502, 0.84496158, 0.03712502],
           [0.00163116, 0.03712502, 0.00163116]])
    >>> harmonc(a, sigma=1)  # more smoothing
    array([[0.05855018, 0.09653293, 0.05855018],
           [0.09653293, 0.15915589, 0.09653293],
           [0.05855018, 0.09653293, 0.05855018]])
    >>> # Several modes are possible for handling boundaries
    >>> harmonc(a, sigma=1, mode='reflect')
    array([[0.08767308, 0.12075024, 0.08767308],
           [0.12075024, 0.16630671, 0.12075024],
           [0.08767308, 0.12075024, 0.08767308]])
    >>> # For RGB images, each is filtered separately
    >>> from skimage.data import astronaut
    >>> image = astronaut()
    >>> filtered_img = harmonc(image, sigma=1, channel_axis=-1)
    ...
    tmp = np.copy(image)
    tmp = np.pad(tmp, 1, "constant", constant_values=0)
    newGray = np.zeros(tmp.shape)
    for i in range(1, tmp.shape[0] - 1):
        for j in range(1, tmp.shape[1] - 1):
            newGray[i, j] = 9.0 / (np.sum(1.0 / tmp[i - 1:i + 2, j - 1:j + 2]))
    return newGray
