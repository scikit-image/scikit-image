...
def test_set_order(dtype, order):
    if order is None:
        # Default order
        assert _set_order(dtype, None) == 0 if dtype == bool else 1
    elif order < 0 or order > 5:
        # Order not in valid range
        with testing.raises(ValueError):
            _set_order(dtype, order)
    elif dtype == bool and order != 0:
        # Deprecated order for bool array
        with expected_warnings(["Input image dtype is bool"]):
            assert _set_order(bool, order) == order
    else:
        # Valid use case
        assert _set_order(dtype, order) == order
