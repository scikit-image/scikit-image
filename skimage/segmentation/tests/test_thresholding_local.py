...
class Test_threshold_local_sauvola:
    def setup_method(self):
        self.image = np.array(
            [
                [0, 0, 1, 3, 5],
    ...
    def test_simple(self):
        ...
        thres = threshold_local_sauvola(self.image, window_size=3, k=0.2, r=128)
        out = self.image > thres
        assert_equal(ref, out)
    ...
    def test_iterable_window_size(self):
        ...
        thres = threshold_local_sauvola(self.image, window_size=(3, 5), k=0.2, r=128)
        out = self.image > thres
        assert_equal(ref, out)
    ...
    def test_dtypes(self, dtype):
        ...
        # use double precision result as a reference
        expected = threshold_local_sauvola(r.astype(float), r=128)

        out = threshold_local_sauvola(r, r=128)
        assert out.dtype == _supported_float_type(dtype)
        assert_allclose(out, expected, rtol=1e-5, atol=1e-5)
