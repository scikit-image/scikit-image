...
def _hessian_matrix_det_and_laplacian(cnp.double_t[:, ::1] img, double sigma):
    ...
    scale normalised. And by definition of Laplacian of Gaussian
    we can get it by summing scale normalised second derivates
    d_xx + d_yy.
    ...
    -------
    (doh, laplacian) : tuple
        doh - The array of the Determinant of Hessians.
        laplacian - The array of Laplacian of Gaussians.
    ...
    cdef Py_ssize_t b = (size - 1) / 2
    cdef cnp.double_t mid, side, tl, tr, bl, br
    cdef cnp.double_t[:, ::1] doh = np.zeros_like(img, dtype=np.double)
    cdef cnp.double_t[:, ::1] laplacian = np.zeros_like(img, dtype=np.double)
    cdef cnp.double_t w_i = 1.0 / size / size
    ...
    for r in range(height):
        for c in range(width):
            ...
            doh[r, c] = (dxx * dyy - 0.81 * (dxy * dxy))
            laplacian[r, c] = dxx + dyy

    return (doh, laplacian)
