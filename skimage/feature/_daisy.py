...
from .. import image_as_float, draw
...
def daisy(image, step=4, radius=15, rings=3, histograms=8, orientations=8,
    ...
    descs_image : (M, N, 3) array (only if visualize==True)
    ...
    if visualize:
        ...
        for i in range(descs.shape[0]):
            for j in range(descs.shape[1]):
                ...
                draw.set_color(descs_image, (rows, cols), color, alpha=val)
