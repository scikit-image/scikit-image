...
def masked_register_translation(src_image, target_image, src_mask, target_mask=None, overlap_ratio=3/10):
    """
    ...
    target_image : ndarray
        Image to register.  Must be same dimensionality as ``src_image``, but not necessarily the same size.
    src_mask : ndarray
        Boolean mask for ``src_image``. The mask should evaluate to ``True``
		(or 1) on valid pixels. ``src_mask`` should have the same shape as ``src_mask``.
    target_mask : ndarray or None, optional
        Boolean mask for ``target_image``. The mask should evaluate to ``True``
		(or 1) on valid pixels. ``target_mask`` should have the same shape as ``target_image``.
        If ``None``, ``src_mask`` will be used.
    overlap_ratio : float, optional
        Maximum allowed overlap ratio between masks. The correlation at pixels with overlap ratio higher
        than this threshold will be zeroed. TODO: specify effect of smaller vs larger overlap_ratio
    ...
    for (im, mask) in [(src_image, src_mask), (target_image, target_mask)]:
        if im.shape != mask.shape:
            raise ValueError("Error: image sizes must match their respective mask sizes.")
    
    # The mismatch in size will impact the center location of the cross-correlation
