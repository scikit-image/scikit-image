...
def _glcm_loop_mask(any_int[:, ::1] image,
                    cnp.uint8_t[:, ::1] mask,
                    cnp.float64_t[:] distances,
                    cnp.float64_t[:] angles, Py_ssize_t levels,
                    cnp.uint32_t[:, :, :, ::1] out):
    """Perform selective co-occurrence matrix accumulation using a binary mask.
    ...
    image : ndarray
        ...
        If type is other than uint8, the argument `levels` needs to be set.
    mask : ndarray
        Boolean mask array. Pixels corresponding to True values in `mask`
            ...
            for d_idx in range(distances.shape[0]):
                ...
                for r in range(start_row, end_row):
                    for c in range(start_col, end_col):
                        row = r + offset_row
                        col = c + offset_col
                        if not mask[r, c] and not mask[row, col]:
                            i = image[r, c]
                            j = image[row, col]
                            if 0 <= i < levels and 0 <= j < levels:
                                out[i, j, d_idx, a_idx] += 1
