...
@pytest.mark.skipif(not is_installed('networkx'),
                    reason="networkx not installed")
def test_cut_normalized_stability1():
    np.random.seed(349)
    img = data.coffee()
    labels = segmentation.slic(img, compactness=30, n_segments=400)
    rag = graph.rag_mean_color(img, labels, mode='similarity')
    start = time()
    # count, eigen, min_cut, label, _ncut_relabel
    call_count = [0, 0, 0, 0, 0]

    label_gen = graph.cut_normalized_gen(labels, rag, 0, call_count=call_count)
    next(label_gen)
    raise(Exception('Initialized generator time - {}, count - {}, eigen - {}, cut - {}, label - {}, _ncut_relabel - {}'
        .format(time() - start, call_count[0], call_count[1], call_count[2],
            call_count[3], call_count[4])))

    # Tests whether second smallest eigenvector generated by
    # eigsh consistently reproduced
    labels1 = graph.cut_normalized(labels, rag, 1e-7)
    labels2 = label_gen.send(1e-7)
    ...
    labels1 = graph.cut_normalized(labels, rag, 1e-5)
    labels2 = label_gen.send(1e-5)
    labels1, _, _ = segmentation.relabel_sequential(labels1)
    labels2, _, _ = segmentation.relabel_sequential(labels2)
    assert (labels1 == labels2).all()
    ...
    labels1 = graph.cut_normalized(labels, rag, 1e-6)
    labels2 = label_gen.send(1e-6)
    labels1, _, _ = segmentation.relabel_sequential(labels1)
    labels2, _, _ = segmentation.relabel_sequential(labels2)
    assert (labels1 == labels2).all()
...
@pytest.mark.skipif(not is_installed('networkx'),
                    reason="networkx not installed")
def test_cut_normalized_stability2():
    np.random.seed(349)
    img = data.coffee()
    labels = segmentation.slic(img, compactness=30, n_segments=400)
    rag = graph.rag_mean_color(img, labels, mode='similarity')
    ...
    # Tests whether second smallest eigenvector generated by
    # eigsh consistently reproduced
    labels1 = graph.cut_normalized(labels, rag, 1e-7, call_count=call_count)
    labels2 = label_gen.send(1e-7)
    labels1, _, _ = segmentation.relabel_sequential(labels1)
    labels2, _, _ = segmentation.relabel_sequential(labels2)
    assert (labels1 == labels2).all()
    raise(Exception('Got through first test time - {}, count - {}, eigen - {}, cut - {}, label - {}, _ncut_relabel - {}'
        .format(time() - start, call_count[0], call_count[1], call_count[2],
            call_count[3], call_count[4])))
...
@pytest.mark.skipif(not is_installed('networkx'),
                    reason="networkx not installed")
def test_cut_normalized_stability3():
    np.random.seed(349)
    img = data.coffee()
    labels = segmentation.slic(img, compactness=30, n_segments=400)
    rag = graph.rag_mean_color(img, labels, mode='similarity')
    ...
    labels1 = graph.cut_normalized(labels, rag, 1e-5, call_count=call_count)
    labels2 = label_gen.send(1e-5)
    labels1, _, _ = segmentation.relabel_sequential(labels1)
    labels2, _, _ = segmentation.relabel_sequential(labels2)
    assert (labels1 == labels2).all()
    raise(Exception('Got through first two tests time - {}, count - {}, eigen - {}, cut - {}, label - {}, _ncut_relabel - {}'
        .format(time() - start, call_count[0], call_count[1], call_count[2],
            call_count[3], call_count[4])))
...
#@pytest.mark.skipif(not is_installed('networkx'),
#                    reason="networkx not installed")
#def test_cut_normalized_stability4():
#    np.random.seed(349)
#    img = data.coffee()
#    labels = segmentation.slic(img, compactness=30, n_segments=400)
#    rag = graph.rag_mean_color(img, labels, mode='similarity')
...
#    labels2, _, _ = segmentation.relabel_sequential(labels2)
#    assert (labels1 == labels2).all()
#    raise(Exception('Would have finished'))
