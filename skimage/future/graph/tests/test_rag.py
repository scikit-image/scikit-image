...
def test_cut_normalized_stability1():
    ...
    next(label_gen)
    raise(Exception('Initialized generator {}'.format(time() - start)))
...
def test_cut_normalized_stability2():
    ...
    labels2, _, _ = segmentation.relabel_sequential(labels2)
    assert (labels1 == labels2).all()
    raise(Exception('Got through first test {}'.format(time()-start)))
...
def test_cut_normalized_stability3():
    ...
    labels2, _, _ = segmentation.relabel_sequential(labels2)
    assert (labels1 == labels2).all()
    raise(Exception('Got through first two tests {}'.format(time() - start)))
