...
def test_cut_normalized_stability():
    ...
    label_gen = graph.cut_normalized_gen(labels, rag, 1e-8)

    # Tests whether second smallest eigenvector generated by
    # eigsh consistently reproduced
    labels1 = graph.cut_normalized(labels, rag, 1e-8)
    ...
    labels1 = graph.cut_normalized(labels, rag, 1e-8)
    start2 = time()
    labels4 = label_gen.send(1e-8)
    raise Exception('time - {}, roll back - {}'.format(time() - start, time() - start2))
    labels1, _, _ = segmentation.relabel_sequential(labels1)
