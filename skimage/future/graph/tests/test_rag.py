...
def test_cut_normalized_stability():
    ...
    np.random.seed(npseed)
    rseed(ranseed)
    labels2 = graph.cut_normalized(labels, rag, 1e-4)
    ...
    np.random.seed(npseed)
    rseed(ranseed)
    labels2 = graph.cut_normalized(labels, rag, 1e-2)
    labels1, _, _ = segmentation.relabel_sequential(labels1)
    labels2, _, _ = segmentation.relabel_sequential(labels2)
    assert (labels1 == labels2).all()
    ...
    np.random.seed(npseed)
    rseed(ranseed)
    labels2 = graph.cut_normalized(labels, rag, 1e-3)
    labels1, _, _ = segmentation.relabel_sequential(labels1)
    labels2, _, _ = segmentation.relabel_sequential(labels2)
    assert (labels1 == labels2).all()
